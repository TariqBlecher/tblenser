...
import numpy as np
from HiDisk import HiDisk, solve_for_rdisk

from tblenser.lens_nonparametric_src import LensPoints
from tblenser.utils import rand_sign, sample_inclination_deg
...
for ind, name in enumerate(src_names):
    ...
    for sample in range(n_samples_per_src):
        ...
        rdisk_arcsec = solve_for_rdisk(mhi, z_src=z_src[ind], Rcmol=rcmol)
        hidisk = HiDisk(grid_length_arcsec=grid_length, n_pix=int(grid_length / pix_res_as),
                        rcmol=rcmol, rdisk_arcsec=rdisk_arcsec, smoothing_height_pix=smoothing_height_pix,
                        theta_2_0=theta_20, theta_1_0=theta_10,
                        x_off=np.int(x_off/pix_res_as), y_off=np.int(y_off/pix_res_as), log10_mhi=mhi, z_src=z_src)
        ...
        parameter_tracking[sample] = [mag, rcmol, mhi, x_off, y_off, theta_20, theta_10, rdisk_arcsec]
